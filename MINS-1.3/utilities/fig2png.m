function fig2png(fig, name, size, device, unit)
...
elseif strcmpi(device, 'imwrite');
    frm = getframe(fig); 
    frm = RemoveBlankMargin(frm.cdata, [204, 204, 204]);
    imwrite(frm, sprintf('%s.%s', name, type));
else
    if strcmpi(type, 'pdf') 
